...
if [ "$TARGET" == "ARM64" ]; then
  ...
  echo ""
  echo "Should mvGenTL_Acquire support mvBlueNAOS (Labs) devices (default is 'no')?"
...
if [ "$PCIE_SUPPORT" == "TRUE" ]; then
    ...
    echo "The kernel module MUST be built for mvBlueNAOS (Labs) cameras to be usable!"
    echo "Hit 'n' + <Enter> for 'no', or just <Enter> for 'yes'."
    if [ "$USE_DEFAULTS" == "NO" ] ; then
      read YES_NO
    else
    ...
    if [ "$YES_NO" == "n" ] || [ "$YES_NO" == "N" ]; then
      echo
      echo "Please restart the installer and select 'YES' if you want to use a mvBlueNAOS (Labs) camera!"
      let ERROR_NUMBER=ERROR_NUMBER+1
      echo
    else
...
if [ "$PCIE_SUPPORT" == "TRUE" ]; then
    echo
    echo "Do you want to copy the necessary files to /etc/udev/rules.d for mvBlueNAOS (Labs) device support (default is 'yes')?"
    echo "Hit 'n' + <Enter> for 'no', or just <Enter> for 'yes'."
    if [ "$USE_DEFAULTS" == "NO" ] ; then
      read YES_NO
    else
    ...
    if [ "$YES_NO" == "n" ] || [ "$YES_NO" == "N" ]; then
       echo
       echo 'To be able to use mvBlueNAOS (Labs) devices, copy 51-udev-pcie.rules to /etc/udev/rules.d'
